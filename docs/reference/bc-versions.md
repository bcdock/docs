...
Two values: `Sandbox` and `OnPrem`. `Sandbox` is the common case and matches the online environments you are most likely developing against. `OnPrem` is for when you specifically need the on-premises feature surface to test on-premises behaviour.

Microsoft states these images are for test and development and are not supported in production, and we do not allow production use either - see [reference/limitations § Sandbox and testing use only](limitations.md#sandbox-and-testing-use-only).
