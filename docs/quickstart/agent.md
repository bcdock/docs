...
- Exit codes - `0` ok, `1` general error (including a failed provisioning, or a timed-out `--wait` on create/resume/hibernate), `3` auth, `4` rate-limited, `5` not-found, `124` a wait gave up (`env wait`, `me export --wait`). Documented at <https://docs.bcdock.io/cli/exit-codes/>; full list with `bcdock --help`.
...
- **Parse exit codes** - exit 3 means re-authenticate, exit 5 means the env name didn't resolve, exit 124 means a wait gave up - for `bcdock env wait`, the status you asked for never came (don't loop forever - back off and tell the human). A provisioning failure exits 1: read `bcdock env logs <name> --provisioning`. With `-o json`, every failure is also one JSON object on stderr, with an `error` code to branch on.
