...
BCDock runs Microsoft's Business Central container images. Microsoft's terms for those images state that the image is **"intended and supported by Microsoft only for test and development purposes"** and is **"not supported in production environments"**.

**Our own rule is separate and firmer: you must not use BCDock for live business operations or financial transactions.** That is BCDock's rule about BCDock, not a statement about what Microsoft permits. Microsoft says *not supported*, and we say *not allowed here*.

You also keep whatever Business Central licence your own use requires. See [Microsoft NAV/BC on Docker EULA](https://go.microsoft.com/fwlink/?linkid=861843) for their terms in full; the quotes above are from that document as read 2026-09-06.
...
- **Production hosting** - not supported by Microsoft for these images, and not something we allow. Not happening.
