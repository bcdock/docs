...
The portal, the `bcdock` CLI, CI pipelines and AI agents all call the same public Platform API. What that means for you:

- **Anything the portal can do, the CLI and your agents can do.** No capability is locked inside a UI, so anything you do by hand you can script, put in a pipeline, or hand to an agent.
- **BCDock's own staff tooling uses the same API, not a separate private route.** The same authentication and request logging apply to it as to you.
- **Stable for automation.** CLI commands are stable, exit codes are documented, and every verb supports `--output json`.
